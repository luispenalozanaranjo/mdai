                    ...
                    <div class="row total mini align-items-end">
                        <div class="col-6 col-sm">
                            <div class="input-cont">
                                <label class="label">Nombre checklist</label>
			...
			<mdai-box title="Configuración pregunta">
				...
				<template slot="action">
					<div class="btn-holder text-right">
						<mdai-btn-action class="btn-primary" :class="{'btn-disabled': !validForm}" text="Guardar cambios" v-model="saving" @action="editar" />
					</div>
				</template>
			</mdai-box>
		 </div>
...
<script>
	export default{
		...
		methods: {
			editar: function(){
				if( this.validForm && !this.saving ){
					...
					this.$http({
						method: 'put',
						url: route('checklists.preguntas.update', this.pregunta.id),
		...
		created: function(){
			Object.entries(this.pregunta).forEach(([key, value]) => {
				// Recorremos todos las keys del objeto fields
				this.$set(this.fields, key, value);
			});

			if( this.fields.configuracion === 'salida' ){
				// La pregunta es de salida, hacemos la validacion de campos

				if( this.fields.marca_campo.includes('=') ){
					// Es una operación matemática

					// Separamos los campos y definimos las variables
					let opCampos = this.fields.marca_campo.split('=');
					let marcas = opCampos[1];
					// Seteamos
					this.config.matematica = opCampos[0];
					this.config.logica = 'matematica';

					if( marcas.length ){
						// Recorremos los campos y los agregamos al array
						marcas.split(',').forEach(item => this.camposSalida.push({ name: item }) );
					}
				}
				else{
					// Es un campo normal
					this.config.logica = 'valor';
					this.camposSalida.push({ name: this.fields.marca_campo });
				}
			}
		}
	}
