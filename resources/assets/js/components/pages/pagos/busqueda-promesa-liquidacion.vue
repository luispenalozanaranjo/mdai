			...
			<div v-if="results.data.length">
				<mdai-table :head="tableHead" :body="results.data">
					<template slot-scope="{ item }">
						...
						<td>
							<mdai-status v-if="item.workflow_estado === 3" :id="item.workflow_estado" estado="Pendiente"></mdai-status>
							<mdai-status v-else-if="item.workflow_estado === 13" :id="item.workflow_estado" estado="Pagado"></mdai-status>
							<span v-else>-</span>
						</td>
						<td>
							<div class="row-actions">
								<a :href="route('finanzas.liquidacion.gastos', item.opp)">
				...
				<div class="content-block mt-32" v-if="results.last_page > 1">
...
<script>
	export default{
		data: function(){
			return {
				tableHead: ['OPP', 'RUT', 'Nombre', 'Apellido', 'Proyecto', 'Etapa', 'Tipo', 'Ejecutivo', 'Estado', 'Pago', 'Acciones'],
				fields: {
					opp: "",
					rut: "",
					nombre: "",
