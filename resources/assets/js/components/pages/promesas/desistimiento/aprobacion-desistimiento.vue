						...
						<div class="row total align-items-end">
							<div class="col-12">
								<div class="input-cont checklists">
									<label class="label">Gastos operacionales</label>
									<div class="radio">
										<input id="si1" type="radio" name="enviado" value="con gops" v-model="fields.tiene_gops.value" :disabled="terminado || !permiso" @change="disableTermino">
										<label for="si1">Con GOPS</label>
									</div>
									<div class="radio">
										<input id="no1" type="radio" name="enviado" value="sin gops" v-model="fields.tiene_gops.value" :disabled="terminado || !permiso" @change="disableTermino">
										<label for="no1">Sin GOPS</label>
									</div>
								</div>
							</div>
							<div class="col-12">
								<div class="table">
									<table>
										...
										<tbody>
											<tr>
											<td>{{data.promesa.gops}}</td>
											<td>{{data.promesa.num_cuotas_gops}}</td>
											<td>{{data.promesa.gops_cuota_1}}</td>
											<td>{{data.promesa.gops_cuota_2}}</td>
											<td>{{data.promesa.gop_restante}}</td>
											</tr>
										</tbody>
									</table>
								</div>
	...
	export default{
		...
		methods: {
			...
			terminarActividad: function(){
				...
				this.action = true;
				this.$nodos.terminarActividad({ nodo: this.data.nodo, marca: this.data.promesa.marca, tiene_gops: this.fields.tiene_gops.value }, () => {
