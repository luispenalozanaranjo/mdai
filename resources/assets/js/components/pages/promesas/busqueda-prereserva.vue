<template>
	<div>
				...
				<mdai-box title="Ingresa un parámetro de búsqueda">
					<template slot="content">
						<div class="row total mini align-items-end">
							...
							<div class="col-12 col-sm">
								<mdai-select label="Estado" v-model="fields.estado" :data="estados"></mdai-select>
							</div>
							<div class="w-100"></div>
							<div class="col-12 col-sm">
					...
					<template slot="action">
						<div class="btn-holder text-right">
							<div class="input-cont">
								<mdai-btn-action class="btn-primary" text="Buscar" v-model="searching" @action="getData(1)"></mdai-btn-action>
							</div>
						</div>
					</template>
				</mdai-box>
		...
		<div class="content-block mt-32" v-if="results">
			<div class="mb-16">
				<div class="row align-items-end">
					<div class="col-12 col-sm-6">
						<h3 class="heading-3">Resultados de la búsqueda</h3>
						<p>{{ $methods.showTotal(results.total, 'registro', 'registros') }}</p>
					</div>
			...
			<mdai-table :head="tableHead" :body="results.data">
				<template slot-scope="{ item }">
					...
					<td>
						<mdai-status v-if=" item.estado === 'Promesada' || item.estado === 'Escriturada'" :id="item.workflow_estado ? 13 : 3" :estado="item.workflow_estado ? 'Terminado' : 'Pendiente'"></mdai-status>
					...
					<td>
						<div class="row-actions">
							<a
								...
								"
								v-if="item.workflow && item.marca == 'Vulnerable'"
							>
							...
							<a
								...
								"
								v-if="item.workflow && item.marca == 'Medio'"
							>
								<mdai-tooltip text="Ver detalles" position="top">
									<span class="fa fa-eye"></span>
								</mdai-tooltip>
							...
							<a
								...
								"
								v-if="item.workflow && item.marca == 'Privado'"
							>
								<mdai-tooltip text="Ver detalles" position="top">
									<span class="fa fa-eye"></span>
								</mdai-tooltip>
			...
			<div class="content-block mt-32" v-if="results !== null && results.last_page > 1">
				<mdai-paginador :config="{ total: results.total, per_page: results.per_page, current_page: results.current_page, last_page: results.last_page}"></mdai-paginador>
			</div>
...
export default {
	data: function () {
		return {
			tableHead: ['OPP', 'RUT', 'Nombre', 'Apellido', 'Proyecto', 'Etapa', 'Tipo', 'Ejecutivo', 'Estado', 'Workflow', 'Acciones'],
			fields: {
				opp: "",
				rut: "",
				nombre: "",
	...
	methods: {
		...
		getData: function(page = 1){
			if( !this.searching ){
				...
				this.$store.commit("toggleAction");
				this.$store.commit("addActionText", "Filtrando data...");
                console.log(`/get-promesas${this.$methods.toURL(this.fields)}`);
