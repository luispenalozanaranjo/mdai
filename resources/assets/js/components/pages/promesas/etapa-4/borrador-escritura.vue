			...
			<form @submit.prevent="submit">
				<mdai-box :title="data.nodo.nodo.subactividad">
					<template slot="content">
										...
										<div class="input-cont checkbox">
											<label>
												<input
													...
													v-model="fields.recepcion_escrituracion.value"
													:disabled="terminado || !permiso"
													@change.prevent="disableTermino"
												/>
												<span class="fill-control-indicator"></span>
												<span class="fill-control-description">Pendiente recepción ODE</span>
											</label>
										</div>
									</div>
									<div class="col-4">
										<mdai-input label="Observaciones" v-model="fields.recepcion_escrituracion.observaciones" :readonly="terminado || !permiso"></mdai-input>
									</div>
								</div>
							</li>
							<li>
								<div v-if="fields.recepcion_escrituracion.value" class="row total mini align-items-center">
									<div class="col-4">
										<div class="input-cont checkbox">
											<label>
												<input
													type="checkbox"
													class="fill-control-input"
													v-model="fields.ode_recibido.value"
													:disabled="terminado || !permiso"
													@change.prevent="disableTermino"
												/>
												<span class="fill-control-indicator"></span>
												<span class="fill-control-description">ODE recibido</span>
											</label>
										</div>
									</div>
									<div class="col-4">
										<mdai-input label="Observaciones" v-model="fields.ode_recibido.observaciones" :readonly="terminado || !permiso"></mdai-input>
									</div>
								</div>
							</li>
							<li>
								<div v-if="fields.recepcion_escrituracion.value && fields.ode_recibido.value" class="row total mini align-items-center">
									<div class="col-4">
										<div class="input-cont checkbox">
											<label>
												<input
													type="checkbox"
													class="fill-control-input"
													v-model="fields.ode_revisada_ok.value"
													:disabled="terminado || !permiso"
													@change.prevent="disableTermino"
												/>
												<span class="fill-control-indicator"></span>
												<span class="fill-control-description">ODE revisada OK</span>
											</label>
										</div>
									</div>
									<div class="col-4">
										<mdai-input label="Observaciones" v-model="fields.ode_revisada_ok.observaciones" :readonly="terminado || !permiso"></mdai-input>
									</div>
								</div>
							</li>
							<li v-if="data.promesa.marca !== 'Vulnerable'">
								<div v-if="fields.recepcion_escrituracion.value && fields.ode_recibido.value && fields.ode_revisada_ok.value" class="row total mini align-items-center">
									<div class="col-4">
										<div class="input-cont checkbox">
											<label>
												<input
													type="checkbox"
													class="fill-control-input"
													v-model="fields.instruccion_notarial.value"
													:disabled="terminado || !permiso"
													@change.prevent="disableTermino"
												/>
												<span class="fill-control-indicator"></span>
												<span class="fill-control-description">Instrucción notarial (Opcional)</span>
											</label>
										</div>
									</div>
									<div class="col-4">
										<mdai-input label="Observaciones" v-model="fields.instruccion_notarial.observaciones" :readonly="terminado || !permiso"></mdai-input>
									</div>
								</div>
							</li>
						</ul>
						...
						<div class="mt-30">
							<div class="row total mini align-items-center">
								<div v-if="data.promesa.marca !== 'Vulnerable'" class="col-4">
									<div class="input-cont">
										<label class="label">Nombre notaria</label>
										<div class="cont">
											<input
												type="text"
												v-model="fields.nombre_notaria.value"
		...
		checkTermino: function () {
			...
			Object.entries(this.fields).forEach(([key, content]) => {
				if (this.$methods.checkNullEmpty(content.value) || !content.value) {
					if( key == "instruccion_notarial" || key == "nombre_notaria" ){
					} else {
						incomplete.push(content.nombre);
					}
				}

				if( content.slug !== "observaciones" && content.slug !== "nombre_notaria" && content.slug != "instruccion_notarial" ){
			...
			// this.terminarActividad = (incomplete.length === 0) ? true : false;
			this.terminar = (incomplete.length === 0) ? true : false;
		},
