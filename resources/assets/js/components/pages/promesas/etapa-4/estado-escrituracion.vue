				...
				<mdai-box :title="data.nodo.nodo.subactividad">
					<template slot="content">
						...
						<div class="input-cont checklists">
							...
							<div class="radio">
								<input id="aprobado" type="radio" value="aprobado" name="estado_aprobacion" v-model="fields.estado_escrituracion.value" :disabled="terminado" @change.prevent="disableTermino">
								<label for="aprobado">Aprobado</label>
							</div>
							<div class="radio">
								<input id="rechazado" type="radio" value="rechazado" name="estado_aprobacion" v-model="fields.estado_escrituracion.value" :disabled="terminado" @change.prevent="disableTermino">
								<label for="rechazado">Rechazado</label>
							</div>
							<div class="radio">
								<input id="pendiente" type="radio" value="pendiente" name="estado_aprobacion" v-model="fields.estado_escrituracion.value" :disabled="terminado" @change.prevent="disableTermino">
						...
						<div class="input-cont">
							...
							<div class="cont">
								<textarea name="observaciones" v-model="fields.observaciones.value" :readonly="terminado" required></textarea>
							</div>
						</div>
					</template>
	...
	export default{
		...
		methods: {
			...
			checkTermino: function(){
				...
				if( incomplete.length === 0 && this.fields.estado_escrituracion.value === 'aprobado' ){
					this.terminar = true;
				}
				else{
					this.terminar = false;
			...
			terminarActividad: function(){
				...
				this.action = true;
				this.$nodos.terminarActividad({nodo: this.data.nodo, marca: this.data.promesa.marca, tipo_credito: this.data.promesa.tipo_credito}, () => {
