				...
				<mdai-box :title="data.nodo.nodo.subactividad">
					<template slot="content">
						...
						<div class="row total align-items-center">
							<div v-if="data.promesa.marca =='Vulnerable'" class="col-12 col-sm-4">
								<div class="input-cont checkbox">
									<label>
										<input type="checkbox" class="fill-control-input" v-model="fields.recepcion_escrituracion.value" :disabled="terminado || !permiso" />
										<span class="fill-control-indicator"></span>
										<span class="fill-control-description">Recepción orden escrituración en el CBR</span>
									</label>
								</div>
							</div>
							<div v-if="data.promesa.marca == 'Vulnerable'" class="col-12 col-sm-4">
								<mdai-input label="Fecha de Ingreso CBR" v-model="fields.fecha_ingreso_cbr.value" :readonly="terminado || !permiso"></mdai-input>
							</div>

							<div v-if="data.promesa.marca == 'Vulnerable'" class="col-12 col-sm-4">
								<mdai-input label="Nº Caratula" v-model="fields.num_caratura.value" :readonly="!fields.recepcion_escrituracion.value || terminado || !permiso"></mdai-input>
							</div>
							<div v-if="data.promesa.marca !='Vulnerable'" class="col-12 col-sm-4">
								<div class="input-cont checkbox">
									<label>
										<input
											...
											v-model="fields.ingreso_cbr_ok.value"
											:disabled="terminado || !permiso"
										/>
										<span class="fill-control-indicator"></span>
										<span class="fill-control-description">Ingreso a CBR OK</span>
									</label>
								</div>
							</div>
							...
							<div v-if="data.promesa.marca != 'Vulnerable'" class="col-12 col-sm-4">
								<mdai-input label="Nº Caratula" v-model="fields.num_caratura.value" :readonly="!fields.ingreso_cbr_ok.value || terminado || !permiso"></mdai-input>
							</div>
...
export default {
	...
	methods: {
		...
		terminarActividad: function () {
			...
			this.$nodos.terminarActividad({
				...
				tipo_credito: this.data.promesa.tipo_credito,
			},
			() => {
	...
	created: function () {
		if( this.data.promesa.marca === "Vulnerable" ){
			this.dataFields.push(14);
		}
		else {
			this.dataFields.push(192);
		}
