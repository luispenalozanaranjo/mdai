								...
								<div class="input-cont checkbox">
									<label>
										<input
											...
											v-model="fields.cierre_copias_ok.value"
											:disabled="terminado || !permiso"
											@change.prevent="checkTermino"
										/>
										<span class="fill-control-indicator"></span>
										<span class="fill-control-description" v-text="fields.cierre_copias_ok.nombre"></span>
									</label>
								</div>
							</div>
							<div class="col-4">
								<div class="input-cont checkbox">
									<label>
										<input
											type="checkbox"
											class="fill-control-input"
											v-model="fields.copia_notaria.value"
											:disabled="terminado || !permiso"
											@change.prevent="checkTermino"
										/>
										<span class="fill-control-indicator"></span>
										<span class="fill-control-description" v-text="fields.copia_notaria.nombre"></span>
									</label>
								</div>
							</div>
							<div class="col-4" v-if="data.promesa.marca !== 'Privado'">
								<div class="input-cont checkbox">
									<label>
										<input
											type="checkbox"
											class="fill-control-input"
											v-model="fields.notificar_creacion_carpeta.value"
											:disabled="terminado || !permiso"
											@change.prevent="checkTermino"
										/>
										<span class="fill-control-indicator"></span>
										<span
											...
											v-text="fields.notificar_creacion_carpeta.nombre"
										></span>
									</label>
								</div>
							</div>
...
export default {
	...
	methods: {
		...
		getAlmacen: function () {
			...
			if (this.data.promesa.marca !== "Privado") {
				this.dataFields.push(31);
			}
		...
		checkTermino: function () {
			...
			if (incomplete.length === 0) {
				...
				incomplete.length === 1 &&
				incomplete.includes("notificar_creacion_carpeta")
			) {
				this.terminar = true;
			} else {
				this.terminar = false;
