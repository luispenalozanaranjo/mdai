						...
						<div class="row total align-items-center">
							...
							<div class="col-4">
								<div class="input-cont checklists">
									...
									<div class="radio">
										<input id="si1" type="radio" name="enviado" value="aprobado" v-model="fields.autoriza.value" :disabled="terminado || !permiso" @change="disableTermino">
										<label for="si1">Aprobado</label>
									</div>
									<div class="radio">
										<input id="no1" type="radio" name="enviado" value="rechazado" v-model="fields.autoriza.value" :disabled="terminado || !permiso" @change="disableTermino">
										<label for="no1">Rechazado</label>
									</div>
								</div>
							</div>
							<div class="col-12">
		...
		methods: {
			...
			validateForm: function(){
				...
				if( !this.submitAction ){
					...
					if(this.$methods.checkNullEmpty(this.fields.generar_nueva_cotizacion.value) ||
					  this.$methods.checkNullEmpty(this.fields.solicitar_pre_aprobacion_bancaria.value)||
					  this.$methods.checkNullEmpty(this.fields.autoriza.value)){
			...
			terminarActividad: function(){
				...
				this.action = true;
				this.$nodos.terminarActividad({nodo: this.data.nodo, marca: this.data.promesa.marca, autoriza: this.fields.autoriza.value}, () => {
