					...
					<div class="row total align-items-center">
						<div class="col-4">
							<div class="input-cont checkbox">
								<label>
									<input type="checkbox" class="fill-control-input" v-model="fields.recepcionado.value" :disabled="terminado || !permiso" @change="disableTermino">
									<span class="fill-control-indicator"></span>
									<span class="fill-control-description">Recepcion documentacion cliente</span>
								</label>
							</div>
						</div>
						<div class="col-4">
							<div class="input-cont checkbox" v-if="fields.recepcionado.value">
								<label>
									<input type="checkbox" class="fill-control-input" v-model="fields.promesa_entregada.value" :disabled="terminado || !permiso">
									<span class="fill-control-indicator"></span>
									<span class="fill-control-description">Entrega documentacion cliente</span>
								</label>
							</div>
						</div>
						<div class="col-12">
		...
		methods: {
			...
			validateForm: function(){
				...
				if( !this.submitAction ){
					...
					if(this.$methods.checkNullEmpty(this.fields.promesa_entregada.value)){
					...
					if( this.$methods.checkNullEmpty(this.fields.recepcionado.value) || !this.fields.recepcionado.value ){
						this.errors.push('El check de Recepcionado es requerido.');
					}
			...
			checkTermino: function(){
				...
				if( !this.$methods.checkNullEmpty(this.fields.recepcionado.value) && 
					!this.$methods.checkNullEmpty(this.fields.promesa_entregada.value) &&
			...
			disableTermino: function(){
				this.terminar = false;
				this.fields.promesa_entregada.value = null;
			},
