<template>
	<div>
		...
		<div class="content-block mt-32" v-if="results !== null">
			...
			<mdai-table :head="tableHead" :body="results.data" v-if="results.length">
				<template slot-scope="{ item }">
					...
					<td><mdai-status v-if="item.estado === 'Promesada' || item.estado === 'Escriturada'" :id="item.workflow_estado ? 13 : 3" :estado="item.workflow_estado ? 'Terminado' : 'Pendiente'"></mdai-status>
					<mdai-status v-else id="2" estado="Desistida"></mdai-status></td>
					<td>
						<div class="row-actions">
							<a :href="route('entrega-unidad', item.opp)">
								<mdai-tooltip text="Modificar" position="top">
		...
		methods: {
			...
			getData: function(page = 1){
				if( !this.searching ){
					...
					this.$http.post( route('entrega.unidad.search'), this.fields )
					.then(response => {
